...
class MahamiGlanceWidget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        val all = runCatching { TaskRepository.get(context).all() }.getOrDefault(emptyList())
        val open = all.filter { it.status != TaskStatus.DONE }
            .sortedWith(compareBy({ it.dueDate }, { it.dueTime }))
        val doneCount = all.count { it.status == TaskStatus.DONE }

        provideContent {
            GlanceTheme {
                WidgetBody(open.take(4), open.size, doneCount, all.size)
            }
        }
    }

    @Composable
    private fun WidgetBody(tasks: List<Task>, openCount: Int, doneCount: Int, total: Int) {
        ...
        Column(
            ...
            if (tasks.isEmpty()) {
                ...
                if (openCount > tasks.size) {
                    Text(
                        text = "+${openCount - tasks.size} مهام أخرى",
    ...
    private fun WidgetRow(task: Task, today: LocalDate) {
        ...
        Row(
            ...
            Column(modifier = GlanceModifier.defaultWeight()) {
                ...
                Text(
                    text = if (late) "متأخرة • ${task.dueDate.format(dateFmt)}"
                    else "${task.dueDate.format(dateFmt)} • ${task.category}",
                    maxLines = 1,
                    style = TextStyle(color = ColorProvider(Color(0xFF6B7280)), fontSize = 11.sp)
                )
            }
...
class CompleteTaskAction : ActionCallback {
    override suspend fun onAction(
        ...
        TaskRepository.get(context).markDoneById(id)
