...
## وضعان للتشغيل
- **مستضاف (للنشر العام)**: حسابات + اشتراكات Google Play؛ الخادم في `server/` يحمل مفتاح Anthropic ويفرض حدود كل باقة. دليل النشر الكامل: [`PLAY_STORE.md`](../PLAY_STORE.md).
- **مفتاح خاص (BYOK)**: المستخدم يضع مفتاحه بنفسه؛ هذا هو الوضع الافتراضي عندما لا تُضبط متغيرات `VITE_API_URL` و`VITE_SUPABASE_*`.
